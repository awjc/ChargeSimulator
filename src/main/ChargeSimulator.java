				...
				if(e.getKeyCode() == KeyEvent.VK_5){
					...
					for (int i=0; i < FRAME_WIDTH; i += dx) {
						for (int j=0; j < FRAME_HEIGHT; j += dy) {
							testCharges.add(new TestCharge((i - FRAME_WIDTH/2)/scaleFactor, (j - FRAME_HEIGHT/2)/scaleFactor, true));
						}
					}
				}
...
//	}
	...
	public void paintComponent(Graphics g){
		...
		offG.drawString(String.format("Mouse: (%d, %d), Potential: %.2f", mouseX, mouseY, mag), 10, 20 + fm.getAscent()*2);
	...
	private float getPotentialAt(int xPos, int yPos){
		...
		synchronized(posCharges){
			for(Charge c : posCharges){
				float pot = (float) c.getPotentialAt((xPos - centerPos.width)/scaleFactor, (yPos - centerPos.height)/scaleFactor);
