...
// r.nextInt(FRAME_HEIGHT) - FRAME_HEIGHT
// / 2, 50 + r.nextInt(100)));
...
// if(i % 2 == 0)
// testCharges.add(new TestCharge(r.nextInt(FRAME_WIDTH) - FRAME_WIDTH / 2,
...
// synchronized(posCharges){
// posCharges.add(new Charge((i - FRAME_WIDTH / 2) / scaleFactor, (j -
// FRAME_HEIGHT / 2)
// / scaleFactor, 400));
// }
// }
...
// synchronized(negCharges){
// negCharges.add(new Charge((radius * Math.cos(theta)) / scaleFactor, (radius *
// Math
...
// testCharges.add(new TestCharge((i - FRAME_WIDTH / 2) / scaleFactor, (j -
// FRAME_HEIGHT / 2)
// / scaleFactor, true));
// }
// }
...
// offG.drawString(String.format("Zoom: %.2f%%", scaleFactor * 100), 10, 10 +
// fm.getAscent());
...
// @Override
// public void run(){
// //int gid = ((getGlobalId(1)/pdy)*pdy) * ((getGlobalSize(0)/pdx)*pdx) +
// ((getGlobalId(0)/pdx)*pdx);
// int gid = (getGlobalId(1)/pdy)*(getGlobalSize(0)/pdx+1) +
// (getGlobalId(0)/pdx);
// float val = arr[gid];
// // float val = gid;
...
// cols[getGlobalId(1)*getGlobalSize(0) + getGlobalId(0)] = 0x99000000 | (colVal
// << 16) | (colVal << 8);
// }
// }
...
// // for(int i = 0, ii = 0; i < w; i += pdx, ii++){
// // for(int j = 0, jj = 0; j < h; j += pdy, jj++){
// // float col = (float) (Math.log((float) (arr[jj * (w / pdx + 1) + ii]) /
// Math.log(magMax)));
// // if(col > 1.0f)
// // col = 1.0f;
// // if(col < 0.0f)
...
// final int[] imgBuf = ( (DataBufferInt) res.getRaster().getDataBuffer()
// ).getData();
